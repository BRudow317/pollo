...
const program = new Command()
  .name('mars')
  .description('CLI for querying and moving data between Salesforce orgs and Oracle databases')
  .version('0.1.0')
...
program
  ...
  .catch((err: unknown) => {
    ...
    console.error(process.env.DEBUG ? err : `Error: ${err instanceof Error ? err.message : String(err)}`)
    process.exit(1)
  })
